...
{
	actions.clear();
	if (nodeID.x < width && map[nodeID.x++][nodeID.y] != 'T')
	{
		actions.push_back(oGridAction::RIGHT);
	}
	if (nodeID.x != 0 && map[nodeID.x--][nodeID.y] != 'T')
	{
		actions.push_back(oGridAction::LEFT);
	}
	if (nodeID.y < height && map[nodeID.x][nodeID.y++] != 'T')
	{
		actions.push_back(oGridAction::UP);
	}
	if (nodeID.y > 0 && map[nodeID.x][nodeID.y--] != 'T')
	{
		actions.push_back(oGridAction::DOWN);
	}
	if (nodeID.x < width && nodeID.y < height && map[nodeID.x++][nodeID.y++] != 'T')
	{
		actions.push_back(oGridAction::UPRIGHT);
	}
	if (nodeID.x != 0 && nodeID.y < height && map[nodeID.x--][nodeID.y++] != 'T')
	{
		actions.push_back(oGridAction::UPLEFT);
	}
	if (nodeID.x < width && nodeID.y != 0 && map[nodeID.x++][nodeID.y--] != 'T')
	{
		actions.push_back(oGridAction::DOWNRIGHT);
	}
	if (nodeID.x != 0 && nodeID.y != 0 && map[nodeID.x--][nodeID.y--] != 'T')
	{
